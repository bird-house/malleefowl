...
date
echo "run WPS_evspsblpot_day.sh with filename:" $1
...
cdo subc,273.2  $in_path'tas_'$name     $temp_dir/tas.nc # 
cdo divc,1.3    $in_path'sfcwind_'$name $temp_dir/sfcwind.nc
cdo add         $in_path'rlds_'$name    $in_path'rlus_'$name  $temp_dir/rlds_net.nc
cdo add         $in_path'rsds_'$name    $in_path'rsus_'$name  $temp_dir/rsds_net.nc 
...
cdo -O merge $in_path'ps_'$name $in_path'huss_'$name $temp_dir/merge_$name
cdo expr,'e=((ps*huss)/62.2)' $temp_dir/merge_$name $temp_dir/e_$name

# partial vapour pressure using Magnus-Formula over water 
cdo expr,'es=6.1078*exp(17.08085*(tas-273.16)/(234.175+(tas-273.16)))' $in_path'tas_'$name $temp_dir/es_$name

# calculate relative humidity 

cdo setname,hurs -setreftime,1949-12-01,00:00:00,day -div $temp_dir/e_$name  $temp_dir/es_$name $out_path'hurs_'$name
ncatted -O -a units,hurs,o,c,1 $out_path'hurs_'$name 
ncatted -O -a standard_name,hurs,o,c,"relative_humidity" $out_path'hurs_'$name

cdo mulc,100  $out_path'hurs_'$name $temp_dir/hurs.nc

# dew point temperature
cdo merge $temp_dir/tas.nc $temp_dir/hurs.nc $temp_dir/hurs_tas.nc
cdo expr,'tdps=(((hurs/100)^(1/8))*(112+0.9*tas)+0.1*tas-112)' $temp_dir/hurs_tas.nc $temp_dir'tdps_'$name
...
cdo  addc,243.12 $temp_dir'tdps_'$name   $temp_dir/NTD.nc
cdo  mulc,17.62  $temp_dir'tdps_'$name 	 $temp_dir/ZTD.nc
...
cdo setname,evspsblpot -divc,86400 $temp_dir/ET0.nc $out_path'evspsblpot_'$name 
ncatted -O -a units,evspsblpot,o,c,'kg m-2 s-1' $out_path'evspsblpot_'$name 
ncatted -O -a standard_name,evspsblpot,o,c,"potential_water_evaporation_flux" $out_path'evspsblpot_'$name
